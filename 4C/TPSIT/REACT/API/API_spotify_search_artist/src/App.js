    ...
    const mostraArtista = () => {
        ...
        return artists.map(artist => (
            <div className='row'>
                ...
                {/* popolarità */}
                <div class="col-4 datiArtista">Popolarità: {artist.popularity}%</div>
                {/* followers */}
                <div class="col-4 datiArtista">Followers: {artist.followers.total}</div>
                {/* generi (titolo) */}
                <div class="col-4 linkArtista">Generi:</div>
                {/* generi (map) */}
                {artist.genres.map(genere => (
                    <div class="col-4 datiArtista">·{genere}</div>
                ))}
                {/* link */}
                <div class="col-4 linkArtista"><a href={artist.external_urls.spotify} target="new">VISUALIZZA IN APP</a></div>
            </div>
        ))
    }

//     <div key={artist.id}>
    //     {artist.images.length ? <img width={"100%"} src={artist.images[0].url} alt=""/> : <div>No image <i class="fa fa-sort-amount-desc" aria-hidden="true"></i></div>}
    //     {artist.name}
    //     {artist}
//      </div>
    ...
    return (
        <div className="App">
            <header className="App-header">
                {/* titolo */}
                <h1>React JS ~ Spotify Artists Search</h1>
