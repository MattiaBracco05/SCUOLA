...
const CercaPerCognome = () => {
    ...
    const alertNome = (event) => {
        ...
        for (var i=0; i<8; i++) {
            ...
            if (nomeRicerca == mieiVeicoli[i].cliente.cognome) {
                console.log("trovato")
                flag = 1;
                var prezzo = mieiVeicoli[i].tariffa * mieiVeicoli[i].cliente.giorni
        ...
        //Se il cognome non è stato trovato --> messaggio di errore
        if (flag == 0) {
            setRisulatato([
                <div class="row justify-content-center mt-3">
                    <div class="col-12">Utente non trovato</div>
                </div>])
        }
    ...
    return (
        <div>
            <h1>Cerca per cognome</h1>
            {/* FORM */}
            <form onSubmit={alertNome}>
            
                {/* INPUT TEXT */}
                <label>
                    <input type="text" value={nomeRicerca} placeholder={"Inserisci un cognome"} onChange={(e) => setNomeRicerca(e.target.value)} />
                </label>
...
export default CercaPerCognome